...
function isDevelopmentEnvironment(){
	var response = false;
	var development_domains = ['localhost','neomediax.net','localhost'];
...
var facturacionTelcel = (function(){
	...
	function initListadoDescargaFacturas(){
	  ...
	  //   			$(filter).toggleClass( 'visible' );
		...
		function generarListadoConfirmacionInicial(){
			...
			$elements.each(function (index, value) {
			  ...
			  var $element = $(this);
				...
				if(meta != null){
					var elemento = (
						$listaFacturas.hasClass('facturas-equipos') ?
						generarHTMLFacturacionEquipos(meta, index) :
						($listaFacturas.hasClass('referencia-unica') ?
						generarHTMLReferenciaUnica(meta, index) :
						($listaFacturas.hasClass('pendientes-pago') ?
						generarHTMLPendientesPago(meta, index) :
						($listaFacturas.hasClass('solicitud-descargas') ?
						generarHTMLSolicitudDescargas(meta, index) :
						($listaFacturas.hasClass('complementos-pago') ?
						generarHTMLComplementos(meta, index) :
						generarHTMLFacturacion(meta, index) ) ) ) )
					);
					$element.html(elemento);
					// dataListadoCuentasAsociadas.push(meta);
		...
		function generarHTMLComplementos(meta, index){
			var html = '';

			html = '<div class="col-sm-pr-100 col-xs-12 content-item-block"> <div class="col-sm-pr-5 col-xs-2 checkbox-container flexbox h-align-center"> <input type="checkbox" id="i-'+meta.id+'" name="i-'+meta.id+'" value="'+meta.id+'" data-index="0"> <label for="i-'+meta.id+'"><span class="check-sq"></span> </label> </div> <div class="col-sm-pr-95 col-xs-12 flexbox h-align-center main-content-block"> <div class="col-sm-pr-40 col-xs-12"> <p class="mobile-only hidden-sm hidden-md hidden-lg">Región:</p> <p title="'+meta.folio+'">'+meta.folio+'</p> </div> <div class="'+(CFDI ? 'col-sm-pr-30' : 'col-sm-pr-30')+' col-xs-12"> <p class="mobile-only hidden-sm hidden-md hidden-lg">RFC:</p> <p title="'+meta.rfc+'">'+meta.rfc+'</p> </div> <div class="'+(CFDI ? 'col-sm-pr-25' : 'col-sm-pr-25')+' col-xs-12 hidden-data"> <p class="mobile-only hidden-sm hidden-md hidden-lg">Razón social:</p> <p title="'+meta.razonsocial+'">'+meta.razonsocial+'</p> </div>'+(CFDI ? '' : '' )+'</div> </div> <div class="btn-mobile col-xs-12 hidden-sm hidden-md hidden-lg"> <button class="bton full-width" title="Ver más">Ver más</button> </div>';
			return html;
